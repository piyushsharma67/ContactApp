...
import { addContact, modifyContact } from '../../redux/reducers/reducer'
import { RootState } from '../../redux/store/store'
import { width } from '../../utils/dimension'

const EditScreen=()=>{

    const {selectedContact}=useSelector((state:RootState)=>state.ContactReducer)

    const [details,setDetails]=React.useState({
        phone:selectedContact.phone,
        name:selectedContact.name,
        type:selectedContact.type,
        isWhatsapp:selectedContact.isWhatsapp,
    ...
    function submitt(){
       //@ts-ignore
        dispatch(modifyContact({id:selectedContact.id,docs:{name:details.name,type:details.type,phone:details.phone,isWhatsapp:details.isWhatsapp}}))
    }
    ...
    return (
        <View style={style.container}>
            ...
            <View style={style.details}>
                ...
                <View style={{width:'90%',marginVertical:10}}>
                    <DropdownComponent label="Select contact type" onPress={updatevalues} keyType={keys.TYPE} val={details.type}/>
                </View>
            ...
            <View style={style.buttonContainer}>
                <TouchableOpacity style={style.button} onPress={submitt}>
                    <Text style={{fontSize:18,color:'white'}}>Update Contact</Text>
                </TouchableOpacity>
            </View>
        </View>
    )
